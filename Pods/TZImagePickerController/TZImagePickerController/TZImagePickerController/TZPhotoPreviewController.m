...
- (void)viewDidLoad {
    ...
    if (!_didSetIsSelectOriginalPhoto) {
        _isSelectOriginalPhoto = _tzImagePickerVc.isSelectOriginalPhoto || _tzImagePickerVc.onlyPickingOriginalPhoto;
    }
...
- (void)configCustomNaviBar {
    ...
    [_backButton setImage:[[UIImage tz_imageNamedFromMyBundle:@"navi_back"] checkOverturn] forState:UIControlStateNormal] ;
...
- (void)viewDidLayoutSubviews {
    ...
    [_backButton resetFrameToFitRTL];
    [_selectButton resetFrameToFitRTL];
    [_indexLabel resetFrameToFitRTL];
    ...
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        CGFloat fullImageWidth = [_tzImagePickerVc.fullImageBtnTitleStr boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} context:nil].size.width + 5;
        _originalPhotoButton.frame = CGRectMake(0, 0, fullImageWidth + 60, 44);
        
//        CGSize titleSize = _originalPhotoButton.titleLabel.bounds.size;
//        _originalPhotoButton.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, -titleSize.width);

        _originalPhotoLabel.frame = CGRectMake(fullImageWidth + 42, 0, 80, 44);
        [_originalPhotoButton resetFrameToFitRTL];
        [_originalPhotoLabel resetFrameToFitRTL];
    }
    ...
    [_doneButton resetFrameToFitRTL];
    [_numberImageView resetFrameToFitRTL];
    [_numberLabel resetFrameToFitRTL];
...
- (void)originalPhotoButtonClick {
    ...
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    if(tzImagePickerVc.onlyPickingOriginalPhoto) {
        return;
    }
